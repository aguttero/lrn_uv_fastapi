...
@app.get("/items4/")
async def read_items_add_title(
...
@app.get("/items5/")
async def read_items_alias_deprecated(
    q: Annotated[
        str | None,
        Query(
            alias="item-query",
...
@app.get("/items6/")
async def read_items_custom_validator(
...
# Example declartion -> model_config
...
async def update_item_example(item_id: int, item: Item2):
    results = {"item_id": item_id, "item": item}
    return results

# Example declaration ->  Field
class Item3(BaseModel):
    name: str = Field(examples=["Foo"])
    description: str | None = Field(default=None, examples=["A field nice Item"])
    price: float = Field(examples=[35.4])
    tax: float | None = Field(default=None, examples=[3.2])


@app.put("/items11/{item_id}")
async def update_item_field_example(item_id: int, item: Item3):
    results = {"item_id": item_id, "item": item}
    return results
